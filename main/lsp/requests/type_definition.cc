    ...
    if (result.error) {
        ...
        if (!queryResponses.empty()) {
            ...
            if (resp->isConstant() || (fileIsTyped && (resp->isIdent() || resp->isLiteral()))) {
                auto tpo = resp->getTypeAndOrigins();
                for (auto loc : locsForType(*gs, tpo.type)) {
                    addLocIfExists(*gs, result, loc);
                }
            } else if (fileIsTyped && resp->isDefinition()) {
                // TODO(jez) Not clear what go to type def on a definition should do...
                // This could be either a method def or a class def
                //
                // Is it fine to do the same thing as what normal go to def would do? Like:
